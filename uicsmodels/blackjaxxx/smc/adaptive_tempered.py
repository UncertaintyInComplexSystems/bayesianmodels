...
from . import tempered  # import blackjax.smc.tempered as tempered
...
def adaptive_tempered_smc(  # renamed from as_top_level_api
    logprior_fn: Callable,
    loglikelihood_fn: Callable,
    mcmc_step_fn: Callable,
    mcmc_init_fn: Callable,
    ...
    **extra_parameters,
    ...
    jax.debug.print('using modded adaptive_tempered_smc')

    mcmc_step_fn_module = inspect.getmodule(mcmc_step_fn)
    if 'uicsmodels' not in mcmc_step_fn_module.__file__:
        jax.debug.print('modded adaptive_tempered_smc called with blackjax step function.\nUse unmodified BlackJax SMC sampler instead.')
        # probably better to use .__package__
